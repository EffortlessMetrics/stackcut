...
- [ ] 1. Contract drift fixes (xtask alias, CI, config parser, docs-check)
  - [ ] 1.1 Create `.cargo/config.toml` with `xtask` alias
    - Add `[alias] xtask = "run --package xtask --"` so `cargo xtask` works as documented
    - _Requirements: 1.1, 1.2_

  - [ ] 1.2 Harden config parser in `stackcut-core`
  ...
  - [ ] 1.5 Add CI workflow tool installation steps
    - Add `cargo install cargo-mutants` and `cargo install cargo-fuzz` with caching to `.github/workflows/ci.yml`
    - _Requirements: 2.1, 2.2, 2.3_

- [ ] 2. Checkpoint — contract drift
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 3. Override engine — parsing and validation
  - [ ] 3.1 Implement `validate_overrides()` in `stackcut-core`
  ...
  - [ ]* 3.2 Write property test for override validation
    - **Property 21: Override Validation Warnings**
    - **Validates: Requirements 6.2, 6.3, 6.4**

- [ ] 4. Override engine — application verbs
  - [ ] 4.1 Add cycle detection to `must_order` in `apply_overrides()`
  ...
  - [ ]* 4.2 Write property tests for override application
...
- [ ] 5. Checkpoint — override engine
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 6. Validate command promotion and exit codes
  - [ ] 6.1 Add `ExitCode` enum and refactor CLI main to use stable exit codes
  ...
  - [ ] 6.2 Promote `cmd_validate` with version/schema enforcement and fingerprint check
  ...
  - [ ] 6.3 Add `--dry-run` flag to `materialize` subcommand
    - Wire `--dry-run` through to `stackcut_git::materialize_patches`
    - _Requirements: 19.1_

  - [ ]* 6.4 Write property test for unsupported plan version rejection
    - **Property 22: Unsupported Plan Version Rejection**
    - **Validates: Requirements 13.1**

- [ ] 7. Planner deepening
  - [ ] 7.1 Emit `prep-refactor` slice kind for rename-only mechanical slices
  ...
  - [ ]* 7.2 Write property test for prep-refactor vs mechanical kind
    - **Property 13: prep-refactor vs Mechanical Kind**
    - **Validates: Requirements 14.1, 14.2**

  - [ ] 7.3 Add review budget diagnostic
  ...
  - [ ]* 7.4 Write property test for review budget diagnostic
    - **Property 14: Review Budget Diagnostic**
    - **Validates: Requirements 15.1**

  - [ ] 7.5 Strengthen ownership inference for docs/tests/generated files
  ...
  - [ ]* 7.6 Write property test for ownership inference
    - **Property 15: Ownership Inference Attachment**
    - **Validates: Requirements 16.1, 16.2, 16.3**

  - [ ] 7.7 Model unsupported Git surfaces in the planner
  ...
  - [ ]* 7.8 Write property test for unsupported surface handling
    - **Property 16: Unsupported Surface Handling**
    - **Validates: Requirements 17.1, 17.2, 17.3, 17.4, 17.5**

- [ ] 8. Checkpoint — planner deepening
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 9. Git edge hardening
  - [ ] 9.1 Handle rename-with-edit, copy, and type-change in `collect_edit_units`
  ...
  - [ ]* 9.2 Write property tests for rename/copy/type-change classification
    - **Property 17: Rename Similarity Classification**
    - **Validates: Requirements 18.1**
    - **Property 18: Copy and Type-Change Classification**
    - **Validates: Requirements 18.2, 18.3**

  - [ ] 9.3 Implement dry-run materialization with rollback
...
- [ ] 10. Checkpoint — git edge hardening
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 11. Artifact fingerprint and richer diagnostics
  - [ ] 11.1 Add `fingerprint` field to `Plan` struct
  ...
  - [ ] 11.2 Implement `compute_fingerprint()` in `stackcut-artifact`
  ...
  - [ ]* 11.3 Write property test for fingerprint verification
    - **Property 24: Fingerprint Verification**
    - **Validates: Requirements 24.1**

  - [ ] 11.4 Implement `DiagnosticsEnvelope` and `write_diagnostics_envelope()` in `stackcut-artifact`
  ...
  - [ ]* 11.5 Write property test for diagnostics envelope completeness
...
- [ ] 13. Proof surface — round-trip properties and planner invariants
  - [ ] 13.1 Add `proptest` dev-dependency to `stackcut-core`, `stackcut-artifact`, `stackcut-git`, `stackcut-cli`
    - Add `proptest` to `[workspace.dependencies]` and each crate's `[dev-dependencies]`
    - _Requirements: 21.1_

  - [ ] 13.2 Implement proptest `Arbitrary` generators in `stackcut-core`
  ...
  - [ ]* 13.3 Write property tests for planner invariants in `stackcut-core`
  ...
  - [ ]* 13.4 Write round-trip property tests in `stackcut-artifact`
...
- [ ] 14. Checkpoint — property tests
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 15. Proof surface — golden tests, snapshot tests, and integration tests
  - [ ] 15.1 Implement fixture-driven golden tests
  ...
  - [ ]* 15.2 Write property test for golden fixture match
    - **Property 25: Golden Fixture Match**
    - **Validates: Requirements 20.1**

  - [ ]* 15.3 Write snapshot tests for artifacts
  ...
  - [ ] 15.4 Implement temp-repo integration tests
  ...
  - [ ]* 15.5 Write property test for end-to-end recomposition
    - **Property 26: End-to-End Recomposition**
    - **Validates: Requirements 23.3**

- [ ] 16. Final wiring and integration
  - [ ] 16.1 Wire `parse_config` into CLI `cmd_plan`
    - Replace `load_toml_or_default::<StackcutConfig>` with `parse_config` call
    - Print config diagnostics to stderr
    - _Requirements: 4.1, 4.2, 4.3_

  - [ ] 16.2 Wire fingerprint verification into `cmd_validate`
    - After loading plan, verify fingerprint if present
    - Emit warning on mismatch
    - _Requirements: 24.2_

  - [ ] 16.3 Wire diagnostics envelope into `cmd_plan`
    - Replace `write_diagnostics` with `write_diagnostics_envelope`
    - _Requirements: 25.1, 25.2_

  - [ ] 16.4 Update fixture `expected.plan.json` files for new `fingerprint` field
